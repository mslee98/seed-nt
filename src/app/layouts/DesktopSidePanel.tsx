...
import { DeviceContextDevPanel } from '../../features/pwa/components/DeviceContextDevPanel'
...
export function DesktopSidePanel() {
  ...
  const showInstallGuide = !isInstalled
  const showDevPanel = import.meta.env.DEV

  if (!showInstallGuide && !showDevPanel) return null
  ...
  return (
    <div className="sticky top-[var(--sticky-header-height)] flex w-full flex-col gap-x6 pt-x6">
      ...
      {showDevPanel && (
        <VStack align="center" className="w-full pl-x4">
          <DeviceContextDevPanel />
        </VStack>
      )}
    </div>
  )
